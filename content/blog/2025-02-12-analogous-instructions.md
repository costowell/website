...
It's not meant to do much, hence the name, but one of the reasons I decided to make it is to do all the codegen myself _including all instruction encoding_.
...
Let's take an instruction that copies the value in the register `ecx` to the register `eax`.
...
These vary from the size of the operands (16-bit, 32-bit, etc.), if we're copying from a register to a location in memory, or even if we're copying a constant into a register.
...
But why stop there? Let's encode our own!
...
The second byte is called the ModR/M byte, and it's what will hold our parameters. So let's go through it piece by piece.
...
Let's start with `0x89` which is specified as follows.
...
Let's try it with the other `mov` opcode, `0x8B`. This time `eax` goes in the `reg` field, and `ecx` goes in the `r/m` field.
